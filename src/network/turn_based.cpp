...
void notify_player_joins(sys::state& state, sys::player_name name, dcon::nation_id nation, sys::player_password_raw password) {
	...
	for(auto cl : state.network_state.clients) {
		...
		socket_add_to_send_queue(cl.send_buffer, &c, sizeof(c));
#ifndef NDEBUG
		state.console_log("host:send:cmd | type:notify_player_joins nation:" + std::to_string(nation.index()) +
		" to:" + std::to_string(cl.playing_as.index()));
#endif
	}
	command::execute_command(state, c);
...
void send_savegame(sys::state& state, client_data& client, bool hotjoin = false) {
	...
	/* Send the savefile to the newly connected client (if not a new game) */
	{
		...
		broadcast_save_to_clients(state, c, state.network_state.current_save_buffer.get(), state.network_state.current_save_length, state.network_state.current_save_checksum);
#ifndef NDEBUG
...
void broadcast_savegame(sys::state& state) {
	for(auto& client : state.network_state.clients) {
		if(!client.is_active()) {
			continue;
		}
		...
		/* Send the savefile */
		{
			...
			socket_add_to_send_queue(client.send_buffer, state.network_state.current_save_buffer.get(), size_t(state.network_state.current_save_length));
#ifndef NDEBUG
					state.console_log("host:send:save | to" + std::to_string(client.playing_as.index()) + " len: " + std::to_string(uint32_t(state.network_state.current_save_length)));
#endif
		}
...
void send_to_client(sys::state& state, command::payload& c) {
	if(c.type == command::command_type::save_game)
		return;
	assert(c.type != command::command_type::notify_save_loaded);
	...
	for(auto& client : state.network_state.clients) {
		if(client.is_active() && (c.source == client.playing_as || c.source == state.local_player_nation || istruebroadcast)) {
			...
			state.console_log("host:send:cmd | to: " + std::to_string(client.playing_as.index()) +
				"from " + std::to_string(c.source.index()) + " type:" + readableCommandTypes[(uint32_t(c.type))]);
#endif
		}
	}
}
		...
		if(state.network_state.save_slock.try_lock()) {
			...
			while(c) {
				if(!command::is_console_command(c->type)) {
					...
					if(c->type != command::command_type::notify_pause_game)
						command::execute_command(state, *c);
					send_to_client(state, *c);
